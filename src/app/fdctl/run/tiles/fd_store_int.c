...
#define NON_SHRED_LINK  3 /* stake, repair and replay are the 3 links not from shred tile */
...
during_frag( fd_store_tile_ctx_t * ctx,
  ...
  fd_store_in_ctx_t * shred_in = &ctx->shred_in[ in_idx-NON_SHRED_LINK ];
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->shred_in_cnt = tile->in_cnt-NON_SHRED_LINK;
  for( ulong i = 0; i<ctx->shred_in_cnt; i++ ) {
    fd_topo_link_t * shred_in_link = &topo->links[ tile->in_link_id[ i+NON_SHRED_LINK ] ];
