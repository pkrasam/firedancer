...
FD_STATIC_ASSERT( FD_GROOVE_DATA_HDR_TYPE_BLOCK     ==0x0fd67, unit_test );
FD_STATIC_ASSERT( FD_GROOVE_DATA_HDR_TYPE_SUPERBLOCK==0x1fd67, unit_test );
...
static fd_groove_volume_t * volume0           = NULL;
...
grow_data( fd_groove_data_t * data,
  ...
  FD_TURNSTILE_BEGIN( 1 /* blocking */ ) {
    int rot = (int)(fd_rng_uint( rng ) & 63U);
    v = fd_ulong_rotate_left( fd_ulong_lsb( fd_ulong_rotate_right( volume_avail_pmap, rot ) ), rot );
    volume_avail_pmap ^= v;
  ...
  ulong idx = (ulong)fd_ulong_find_lsb( v );
  FD_LOG_NOTICE(( "Adding volume %lu to groove", idx ));
...
tile_main( int     argc,
  ...
  while( !FD_VOLATILE_CONST( _go ) ) FD_SPIN_PAUSE();
...
main( int     argc,
  ...
  ulong        volume_cnt = fd_env_strip_cmdline_ulong( &argc, &argv, "--volume-cnt", NULL,             4UL );
  char const * _page_sz   = fd_env_strip_cmdline_cstr ( &argc, &argv, "--page-sz",    NULL,      "gigantic" );
  ulong        near_cpu   = fd_env_strip_cmdline_ulong( &argc, &argv, "--near-cpu",   NULL, fd_log_cpu_id() );
  ulong        alloc_cnt  = fd_env_strip_cmdline_ulong( &argc, &argv, "--alloc-cnt",  NULL,         65536UL );
  ...
  for( ulong rem=10000000UL; rem; rem-- ) {
    uchar block[ FD_GROOVE_DATA_BLOCK_FOOTPRINT ] __attribute__((aligned(FD_GROOVE_DATA_BLOCK_ALIGN)));
    ...
    uchar * mem = block + fd_ulong_max( 1UL+(r2 & (FD_GROOVE_DATA_BLOCK_FOOTPRINT-1UL)), FD_GROOVE_DATA_HDR_FOOTPRINT );

    fd_groove_data_hdr_t * _hdr = fd_groove_data_block_hdr( mem );
    FD_TEST( _hdr==fd_groove_data_block_hdr_const( mem ) );
    FD_TEST( _hdr==(fd_groove_data_hdr_t *)block    );

    fd_groove_data_hdr_t h0 = *_hdr;

    ulong type  = fd_groove_data_hdr_type ( h0 ); FD_TEST( type <  131072UL );
    ulong szc   = fd_groove_data_hdr_szc  ( h0 ); FD_TEST( szc  <     128UL );
    ulong idx   = fd_groove_data_hdr_idx  ( h0 ); FD_TEST( idx  <      64UL );
    ulong sz    = fd_groove_data_hdr_sz   ( h0 ); FD_TEST( sz   <16777216UL );
    ulong align = fd_groove_data_hdr_align( h0 ); FD_TEST( align<    1024UL );
    ulong info  = fd_groove_data_hdr_info ( h0 ); /* arb */

    fd_groove_data_hdr_t h1;

    h1 = fd_groove_data_hdr( type, szc, idx, align, sz, info );
    ...
    FD_TEST( fd_ulong_is_aligned( block_footprint, FD_GROOVE_DATA_BLOCK_ALIGN )                    );
    FD_TEST( (FD_GROOVE_DATA_BLOCK_FOOTPRINT<=block_footprint) & (block_footprint<=(1UL<<25))      );

    FD_TEST( (2UL<=block_cnt) & (block_cnt<=64UL)                                                  );
    FD_TEST( (FD_GROOVE_DATA_BLOCK_FOOTPRINT+block_cnt*block_footprint)<=FD_GROOVE_VOLUME_DATA_MAX );
    ...
    ulong footprint_min = szc==0UL ? 0UL : 1UL+(ulong)fd_groove_data_szc_cfg[ szc-1UL ].block_footprint;
    ulong footprint_max = block_footprint;
    ...
    idx %= fd_groove_data_szc_cfg[ szc ].block_cnt;

    FD_TEST( _hdr==fd_groove_data_superblock_hdr      ( mem+FD_GROOVE_DATA_BLOCK_FOOTPRINT+idx*block_footprint, szc, idx ) );
    FD_TEST( _hdr==fd_groove_data_superblock_hdr_const( mem+FD_GROOVE_DATA_BLOCK_FOOTPRINT+idx*block_footprint, szc, idx ) );
  }
  ...
  fd_groove_data_free( NULL, data ); /* logs warning (NULL data) */
  fd_groove_data_free( data, NULL ); /* no-op (free nothing) */

  /* Start up remote tiles */

  volume0           = volume;
  volume_avail_pmap = fd_ulong_if( volume_cnt<63UL, (1UL<<volume_cnt) - 1UL, ~0UL);
  ...
  /* Wait ~0.1 seconds and send go code */
  ...
  for( ulong idx=0UL; idx<TEST_SLOT_MAX; idx++ ) if( test_slot[ idx ].mem ) fd_groove_data_free( data, test_slot[ idx ].mem );
