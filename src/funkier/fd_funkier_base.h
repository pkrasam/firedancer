#ifndef HEADER_fd_src_funk_fd_funkier_base_h
#define HEADER_fd_src_funk_fd_funkier_base_h
...
/* Funk terminology / concepts:
   ...
   - keys are a fixed length fd_funkier_rec_key_t.
...
/* FD_FUNKIER_SUCCESS is used by various APIs to indicate the operation
   successfully completed.  This will be 0.  FD_FUNKIER_ERR_* gives a
   number of error codes used by fd_funk APIs.  These will be negative
   integers. */

#define FD_FUNKIER_SUCCESS    (0)  /* Success */
#define FD_FUNKIER_ERR_INVAL  (-1) /* Failed due to obviously invalid inputs */
#define FD_FUNKIER_ERR_XID    (-2) /* Failed due to transaction id issue (e.g. xid present/absent when it should be absent/present) */
#define FD_FUNKIER_ERR_KEY    (-3) /* Failed due to record key issue (e.g. key present/absent when it should be absent/present) */
#define FD_FUNKIER_ERR_FROZEN (-4) /* Failed due to frozen issue (e.g. attempt to change records in a frozen transaction) */
#define FD_FUNKIER_ERR_TXN    (-5) /* Failed due to transaction map issue (e.g. funk txn_max too small) */
#define FD_FUNKIER_ERR_REC    (-6) /* Failed due to record map issue (e.g. funk rec_max too small) */
#define FD_FUNKIER_ERR_MEM    (-7) /* Failed due to wksp issue (e.g. wksp too small) */
#define FD_FUNKIER_ERR_SYS    (-8) /* Failed system call (e.g. a file write) */

/* FD_FUNKIER_REC_KEY_{ALIGN,FOOTPRINT} describe the alignment and
   footprint of a fd_funkier_rec_key_t.  ALIGN is a positive integer power
   of 2.  FOOTPRINT is a multiple of ALIGN.  These are provided to
   facilitate compile time declarations. */

#define FD_FUNKIER_REC_KEY_ALIGN     (8UL)
#define FD_FUNKIER_REC_KEY_FOOTPRINT (40UL) /* 32 byte hash + 8 byte meta */

/* A fd_funkier_rec_key_t identifies a funk record.  Compact binary keys
   are encouraged but a cstr can be used so long as it has
   strlen(cstr)<FD_FUNKIER_REC_KEY_FOOTPRINT and the characters c[i] for i
   in [strlen(cstr),FD_FUNKIER_REC_KEY_FOOTPRINT) zero.  (Also, if encoding
...
union __attribute__((aligned(FD_FUNKIER_REC_KEY_ALIGN))) fd_funkier_rec_key {
  char  c [ FD_FUNKIER_REC_KEY_FOOTPRINT ];
  uchar uc[ FD_FUNKIER_REC_KEY_FOOTPRINT ];
  ulong ul[ FD_FUNKIER_REC_KEY_FOOTPRINT / sizeof(ulong) ];
};

typedef union fd_funkier_rec_key fd_funkier_rec_key_t;

/* FD_FUNKIER_TXN_XID_{ALIGN,FOOTPRINT} describe the alignment and
   footprint of a fd_funkier_txn_xid_t.  ALIGN is a positive integer power
   of 2.  FOOTPRINT is a multiple of ALIGN.  These are provided to
   facilitate compile time declarations. */

#define FD_FUNKIER_TXN_XID_ALIGN     (8UL)
#define FD_FUNKIER_TXN_XID_FOOTPRINT (16UL)

/* A fd_funkier_txn_xid_t identifies a funk transaction.  Compact binary
   identifiers are encouraged but a cstr can be used so long as it has
   strlen(cstr)<FD_FUNKIER_TXN_XID_FOOTPRINT and characters c[i] for i in
   [strlen(cstr),FD_FUNKIER_TXN_KEY_FOOTPRINT) zero.  (Also, if encoding a
...
union __attribute__((aligned(FD_FUNKIER_TXN_XID_ALIGN))) fd_funkier_txn_xid {
  char  c [ FD_FUNKIER_TXN_XID_FOOTPRINT ];
  uchar uc[ FD_FUNKIER_TXN_XID_FOOTPRINT ];
  ulong ul[ FD_FUNKIER_TXN_XID_FOOTPRINT / sizeof(ulong) ];
};

typedef union fd_funkier_txn_xid fd_funkier_txn_xid_t;

/* FD_FUNKIER_XID_KEY_PAIR_{ALIGN,FOOTPRINT} describe the alignment and
   footprint of a fd_funkier_xid_key_pair_t.  ALIGN is a positive integer
   power of 2.  FOOTPRINT is a multiple of ALIGN.  These are provided to
   facilitate compile time declarations. */

#define FD_FUNKIER_XID_KEY_PAIR_ALIGN     (8UL)
#define FD_FUNKIER_XID_KEY_PAIR_FOOTPRINT (56UL)

/* A fd_funkier_xid_key_pair_t identifies a funk record.  It is just
   xid and key packed into the same structure. */

struct fd_funkier_xid_key_pair {
  fd_funkier_txn_xid_t xid[1];
  fd_funkier_rec_key_t key[1];
};

typedef struct fd_funkier_xid_key_pair fd_funkier_xid_key_pair_t;

/* A fd_funkier_t * is an opaque handle of a local join to a funk instance */

struct fd_funkier_private;
typedef struct fd_funkier_private fd_funkier_t;

FD_PROTOTYPES_BEGIN

/* fd_funkier_rec_key_hash provides a family of hashes that hash the key
...
/* fd_funkier_rec_key_eq returns 1 if keys pointed to by ka and kb are
...
/* fd_funkier_rec_key_copy copies the key pointed to by ks into the key
...
/* fd_funkier_txn_xid_hash provides a family of hashes that hash the xid
...
/* fd_funkier_txn_xid_eq returns 1 if transaction id pointed to by xa and
...
/* fd_funkier_txn_xid_copy copies the transaction id pointed to by xs into
...
/* fd_funkier_txn_xid_eq_root returns 1 if transaction id pointed to by x
   is the root transaction.  Assumes x is in the caller's address space
   and valid. */

FD_FN_PURE static inline int
fd_funkier_txn_xid_eq_root( fd_funkier_txn_xid_t const * x ) {
  ulong const * a = x->ul;
  return !(a[0] | a[1]);
}

/* fd_funkier_txn_xid_set_root sets transaction id pointed to by x to the
   root transaction and returns x.  Assumes x is in the caller's address
   space and valid. */

static inline fd_funkier_txn_xid_t *
fd_funkier_txn_xid_set_root( fd_funkier_txn_xid_t * x ) {
  ulong * a = x->ul;
  a[0] = 0UL; a[1] = 0UL;
  return x;
}

/* fd_funkier_xid_key_pair_hash produces a 64-bit hash case for a
...
fd_funkier_xid_key_pair_hash( fd_funkier_xid_key_pair_t const * p,
  ...
  return fd_funkier_rec_key_hash( p->key, seed );
}

/* fd_funkier_xid_key_pair_eq returns 1 if (xid,key) pair pointed to by pa
...
/* fd_funkier_xid_key_pair_copy copies the (xid,key) pair pointed to by ps
...
/* fd_funkier_xid_key_pair_init set the (xid,key) pair p to pair formed
...
/* fd_funkier_strerror converts an FD_FUNKIER_SUCCESS / FD_FUNKIER_ERR_* code
...
#endif /* HEADER_fd_src_funk_fd_funkier_base_h */
