#ifndef HEADER_fd_src_funk_fd_funkier_txn_h
#define HEADER_fd_src_funk_fd_funkier_txn_h
...
/* FD_FUNKIER_TXN_{ALIGN,FOOTPRINT} describe the alignment and footprint of
   a fd_funkier_txn_t.  ALIGN will be a power of 2, footprint will be a
   multiple of align.  These are provided to facilitate compile time
   declarations. */

#define FD_FUNKIER_TXN_ALIGN     (32UL)
#define FD_FUNKIER_TXN_FOOTPRINT (96UL)

/* FD_FUNKIER_TXN_IDX_NULL gives the map transaction idx value used to
...
#define FD_FUNKIER_TXN_IDX_NULL ((ulong)UINT_MAX)

/* A fd_funkier_txn_t is an opaque handle of an in-preparation funk
   transaction.  The details are exposed here to facilitate inlining
   various operations. */

struct __attribute__((aligned(FD_FUNKIER_TXN_ALIGN))) fd_funkier_txn_private {

  /* These fields are managed by the funk's txn_map */

  fd_funkier_txn_xid_t xid;      /* Transaction id, at a minimum, unique among all in-prepare and the last published transaction,
  ...
  uint   parent_cidx;       /* compr map index of the in-prep parent         txn, compr FD_FUNKIER_TXN_IDX_NULL if a funk child */
...
typedef struct fd_funkier_txn_private fd_funkier_txn_t;

/* fd_funkier_txn_map allows for indexing transactions by their xid */

#define POOL_NAME          fd_funkier_txn_pool
#define POOL_ELE_T         fd_funkier_txn_t
#define POOL_IDX_T         uint
#define POOL_NEXT          map_next
#define POOL_IMPL_STYLE    1
#include "../util/tmpl/fd_pool_para.c"

#define MAP_NAME              fd_funkier_txn_map
#define MAP_ELE_T             fd_funkier_txn_t
#define MAP_KEY_T             fd_funkier_txn_xid_t
#define MAP_KEY               xid
#define MAP_KEY_EQ(k0,k1)     fd_funkier_txn_xid_eq((k0),(k1))
#define MAP_KEY_HASH(k0,seed) fd_funkier_txn_xid_hash((k0),(seed))
...
/* fd_funkier_txn_{cidx,idx} convert between an index and a compressed index. */

static inline uint  fd_funkier_txn_cidx( ulong idx ) { return (uint) idx; }
static inline ulong fd_funkier_txn_idx ( uint  idx ) { return (ulong)idx; }

/* fd_funkier_txn_idx_is_null returns 1 if idx is FD_FUNKIER_TXN_IDX_NULL and
   0 otherwise. */

static inline int fd_funkier_txn_idx_is_null( ulong idx ) { return idx==FD_FUNKIER_TXN_IDX_NULL; }
...
/* fd_funkier_txn_query returns a pointer to an in-preparation transaction
   ...
   map==fd_funkier_txn_map( funk, fd_funkier_wksp( funk ) ), xid points to a
...
/* fd_funkier_txn_xid returns a pointer in the local address space of the
...
FD_FN_CONST static inline fd_funkier_txn_xid_t const * fd_funkier_txn_xid( fd_funkier_txn_t const * txn ) { return &txn->xid; }
...
#define FD_FUNKIER_ACCESSOR(field)                     \
FD_FN_PURE static inline fd_funkier_txn_t *            \
fd_funkier_txn_##field( fd_funkier_txn_t const * txn,  \
                        fd_funkier_txn_pool_t * pool ) { \
  ulong idx = fd_funkier_txn_idx( txn->field##_cidx ); \
  if( idx==FD_FUNKIER_TXN_IDX_NULL ) return NULL;      \
  return pool->ele + idx;                              \
}

FD_FUNKIER_ACCESSOR( parent       )
FD_FUNKIER_ACCESSOR( child_head   )
FD_FUNKIER_ACCESSOR( child_tail   )
FD_FUNKIER_ACCESSOR( sibling_prev )
FD_FUNKIER_ACCESSOR( sibling_next )

#undef FD_FUNKIER_ACCESSOR

/* fd_funkier_txn_frozen returns 1 if the in-preparation transaction is
...
fd_funkier_txn_is_frozen( fd_funkier_txn_t const * txn ) {
  return !fd_funkier_txn_idx_is_null( fd_funkier_txn_idx( txn->child_head_cidx ) );
}

/* fd_funkier_txn_is_only_child returns 1 if the in-preparation transaction
   txn if is any only child and 0 if it has one or more siblings.
   Assumes txn points to an in-preparation transaction in the caller's
   address space. */

FD_FN_PURE static inline int
fd_funkier_txn_is_only_child( fd_funkier_txn_t const * txn ) {
  return ( fd_funkier_txn_idx_is_null( fd_funkier_txn_idx( txn->sibling_prev_cidx ) ) ) &
         ( fd_funkier_txn_idx_is_null( fd_funkier_txn_idx( txn->sibling_next_cidx ) ) );
}
...
/* fd_funkier_txn_ancestor returns a pointer in the caller's address space
   ...
   fd_funkier_txn_descendant returns a pointer in the caller's address
   ...
   In code:
     ...
     if( !fd_funkier_txn_ancestor( txn, map ) )
...
fd_funkier_txn_ancestor( fd_funkier_txn_t * txn,
                         fd_funkier_txn_pool_t * pool ) {
  for(;;) {
    if( !fd_funkier_txn_is_only_child( txn ) ) break;
...
FD_FN_PURE static inline fd_funkier_txn_t *
fd_funkier_txn_descendant( fd_funkier_txn_t * txn,
                           fd_funkier_txn_pool_t * pool ) {
  if( !fd_funkier_txn_is_only_child( txn ) ) return NULL;
  for(;;) { /* txn is an only child at this point */
    fd_funkier_txn_t * child = fd_funkier_txn_child_head( txn, pool );
    if( !child || !fd_funkier_txn_is_only_child( child ) ) break;
    txn = child;
  }
  return txn;
}
...
/* fd_funkier_txn_{prepare,publish,cancel,cancel_siblings,cancel_children}
...
/* fd_funkier_txn_prepare starts preparation of a transaction.  The
...
/* fd_funkier_txn_cancel cancels in-preparation transaction txn and any of
   ...
   fd_funkier_txn_cancel_siblings cancels txn's siblings and their
   descendants.

   fd_funkier_txn_cancel_children cancels txn's children and their
...
/* This version of publish just combines the transaction with its
   ...
   Returns FD_FUNKIER_SUCCESS on success or an error code on failure. */
int
...
typedef struct fd_funkier_txn_all_iter fd_funkier_txn_all_iter_t;

void fd_funkier_txn_all_iter_new( fd_funkier_t * funk, fd_funkier_txn_all_iter_t * iter );

int fd_funkier_txn_all_iter_done( fd_funkier_txn_all_iter_t * iter );

void fd_funkier_txn_all_iter_next( fd_funkier_txn_all_iter_t * iter );

fd_funkier_txn_t const * fd_funkier_txn_all_iter_ele_const( fd_funkier_txn_all_iter_t * iter );
...
#endif /* HEADER_fd_src_funk_fd_funkier_txn_h */
